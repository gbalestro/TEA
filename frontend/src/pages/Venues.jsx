    ...
    <div className="locais-container px-4 py-3">
        ...
        <Table borderless hover responsive className="mb-0 align-middle">
          <thead className="bg-light">
            <tr style={{ borderBottom: '2px solid #eee' }}>
              <th className="px-5 py-3 text-center" style={{ width: '30%', fontWeight: '600' }}>{t('venues.venueName')}</th>
              <th className="py-3 text-center" style={{ width: '30%', fontWeight: '600' }}>{t('venues.recentTeam')}</th>
              <th className="py-3 text-center" style={{ width: '25%', fontWeight: '600' }}>{t('venues.totalHours')}</th>
              <th className="py-3 text-center" style={{ width: '15%', fontWeight: '600' }}>{t('venues.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {locais.length > 0 ? locais.map((local) => (
              <tr key={local.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                ...
                <td className="py-4 text-center">
                  <div style={{ fontWeight: '700', fontSize: '1.45rem', color: local.total_hours > 0 ? '#3b82f6' : '#ccc' }}>
      ...
      <Modal show={showModal} onHide={handleClose} centered size="md">
        ...
        <Modal.Body className="p-4">
          <Form>
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text" name="name" value={formData.name} onChange={handleInputChange} placeholder="Ex: Obra Alphaville" className="p-2 border-0 bg-light" style={{ borderRadius: '8px' }} />
            </Form.Group>
            <Form.Group className="mb-4">
              <Form.Label className="small fw-bold text-muted">{t('venues.address').toUpperCase()}</Form.Label>
              <Form.Control type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="Rua exemplo, 123 - Bairro" className="p-2 border-0 bg-light" style={{ borderRadius: '8px' }} />
            </Form.Group>
          </Form>
        </Modal.Body>
        <Modal.Footer className="border-0 pt-0 pb-4 px-4 d-flex justify-content-between">
          ...
          <div>
            <Button variant="light" onClick={handleClose} className="fw-bold px-4 me-2">{t('venues.cancel')}</Button>
